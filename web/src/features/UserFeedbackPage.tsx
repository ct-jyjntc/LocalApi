...
export function UserFeedbackPage() {
  const { locale } = useI18n(); const zh = locale === "zh"; const qc = useQueryClient();
  const query = useQuery({ queryKey: ["user", "feedback"], queryFn: userApi.feedback.list });
  const [createOpen, setCreateOpen] = useState(false); const [selected, setSelected] = useState<FeedbackThread | null>(null);
  const [subject, setSubject] = useState(""); const [body, setBody] = useState(""); const [files, setFiles] = useState<FeedbackAttachment[]>([]);
...
function ThreadDialog({ thread, zh, onClose, onDone }: { thread: FeedbackThread; zh: boolean; onClose: () => void; onDone: () => void }) {
  const [body, setBody] = useState(""); const [files, setFiles] = useState<FeedbackAttachment[]>([]); const open = thread.status === "open";
  const reply = useMutation({ mutationFn: () => userApi.feedback.reply(thread.id, body, files), onSuccess: () => { setBody(""); setFiles([]); onDone(); }, onError: (e: Error) => toast.error(e.message) });
  return <Dialog open onOpenChange={value => { if (!value) onClose(); }}><DialogContent className="max-w-[680px]"><DialogHeader><div className="flex items-center justify-between gap-3 pr-6"><DialogTitle>{thread.subject}</DialogTitle><Badge variant={open ? "success" : "secondary"}>{open ? (zh ? "处理中" : "Open") : (zh ? "已解决" : "Resolved")}</Badge></div><DialogDescription>{new Date(thread.created_at).toLocaleString()}</DialogDescription></DialogHeader><div className="mt-4 max-h-[48vh] space-y-2 overflow-y-auto pr-1">{thread.messages.map(message => <MessageBubble key={message.id} message={message} own={message.sender_type === "user"} />)}</div>{open ? <div className="space-y-3 border-t border-border/50 pt-4"><Textarea placeholder={zh ? "继续回复" : "Reply"} value={body} onChange={e => setBody(e.target.value)} /><AttachmentInput files={files} onChange={setFiles} /><DialogFooter><Button variant="secondary" onClick={onClose}>{zh ? "关闭" : "Close"}</Button><Button disabled={(!body.trim() && !files.length) || reply.isPending} onClick={() => reply.mutate()}>{zh ? "发送回复" : "Send reply"}</Button></DialogFooter></div> : <div className="rounded-md bg-secondary/45 px-3 py-2 text-xs text-muted-foreground">{zh ? "该反馈已解决。如需继续沟通，请等待管理员重新打开。" : "This feedback is resolved. An administrator must reopen it before you can reply."}</div>}</DialogContent></Dialog>;
}
