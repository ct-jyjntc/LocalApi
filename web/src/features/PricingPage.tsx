...
const emptyForm = { model: "", input: "0", output: "0", cacheRead: "0", cacheWrite: "0", reasoningEnabled: false, reasoningEffort: [] as string[], imageInput: false, contextWindow: "", maxOutput: "", enabled: true };
...
export function PricingPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title={zh ? "模型配置" : "Model config"} description={zh ? "价格单位为每 100 万 Token 的额度；推理 Token 默认包含在输出中。开启思考的模型可配置支持的思考档位（reasoning_effort）。" : "Prices are credits per one million tokens; reasoning is included in output by default. Models with thinking enabled can declare supported reasoning efforts (reasoning_effort)."} actions={<Button size="sm" onClick={() => { setForm(emptyForm); setFormOpen(true); }}><Plus />{zh ? "新增配置" : "Add config"}</Button>} />
      <Dialog open={formOpen} onOpenChange={setFormOpen}><DialogContent className="max-w-[640px]"><DialogHeader><DialogTitle>{form.model ? (zh ? "编辑模型配置" : "Edit model config") : (zh ? "新增模型配置" : "Add model config")}</DialogTitle><DialogDescription>{zh ? "配置价格与思考参数；思考参数会随 /v1/models 返回给客户端。" : "Prices and reasoning settings; reasoning is exposed via /v1/models."}</DialogDescription></DialogHeader>
