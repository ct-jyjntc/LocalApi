...
export function PricingPage() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { setForm(emptyForm); toast.success(zh ? "模型价格已保存" : "Model price saved"); refresh(); },
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title={zh ? "模型价格" : "Model pricing"} description={zh ? "价格单位为每 100 万 Token 的额度；推理 Token 默认包含在输出中。" : "Prices are credits per one million tokens; reasoning is included in output by default."} />
      <Card>
        <CardHeader><CardTitle>{zh ? "设置价格" : "Set price"}</CardTitle><CardDescription>{zh ? "未设置有效价格的模型不能被用户 Key 调用，旧管理员 Key 不受影响。" : "User keys cannot call models without an active price; legacy admin keys remain unaffected."}</CardDescription></CardHeader>
        <CardContent className="grid gap-2 md:grid-cols-6">
          ...
          <div className="flex items-end gap-2"><Switch checked={form.enabled} onCheckedChange={(enabled) => setForm({ ...form, enabled })} /><Button size="sm" disabled={!form.model.trim() || save.isPending} onClick={() => save.mutate()}><Plus data-icon="inline-start" />{zh ? "保存" : "Save"}</Button></div>
        </CardContent>
      </Card>
