    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden">
        ...
        <div className={TABLE_HEAD_CLASS}>
          ...
          <span className="hidden w-36 text-right tabular-nums sm:block">
            {t("logs.input")}/{t("logs.output")}/{t("logs.cache")}/
            {t("logs.reasoning")}
          </span>
        ...
        {!data?.recent?.length ? (
          ...
          data.recent.map((r) => (
            <div key={r.id} className={TABLE_ROW_CLASS}>
              ...
              <span className="hidden w-36 text-right tabular-nums text-[11px] text-muted-foreground sm:block">
                {(r.prompt_tokens ?? 0).toLocaleString()}/
                {(r.completion_tokens ?? 0).toLocaleString()}/
                {(r.cached_tokens ?? 0).toLocaleString()}/
                {(r.reasoning_tokens ?? 0).toLocaleString()}
              </span>
              <span className="w-36 text-right text-[11px] text-muted-foreground">
