...
function ledgerType(type: string, zh: boolean) { const labels: Record<string, string> = zh ? { payment_topup: "充值入账", payment_refund: "退款扣回", plan_purchase: "购买套餐", plan_upgrade: "升级套餐", plan_upgrade_credit: "升级预付余额退回", plan_renewal: "套餐续费", usage: "API 用量", adjustment: "管理员调整", points_exchange: "积分兑换" } : { payment_topup: "Top-up", payment_refund: "Refund", plan_purchase: "Plan purchase", plan_upgrade: "Plan upgrade", plan_upgrade_credit: "Upgrade credit", plan_renewal: "Plan renewal", usage: "API usage", adjustment: "Adjustment", points_exchange: "Points exchange" }; return labels[type] || type; }
