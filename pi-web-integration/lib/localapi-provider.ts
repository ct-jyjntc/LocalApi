/**
 * LocalApi OAuth provider for Pi-Web (deliverable — see README.md in this
 * directory for merge instructions).
 *
 * Single owner of the LocalApi integration: OAuth broker login (login_url +
 ...
 *
 * The provider contract mirrors lib/atomgit-provider.ts. Server side:
 * LocalApi implements the endpoints under /oauth/* — see the LocalApi repo,
 * docs/pi-web-oauth.md.
 */
import {
  createProvider,
...
import {
  LOCALAPI_BASE_URL,
  LOCALAPI_CHECK_URL,
  LOCALAPI_CODING_BASE_URL,
  LOCALAPI_DISPLAY_NAME,
  LOCALAPI_LOGIN_URL,
  LOCALAPI_PROVIDER_ID,
  LOCALAPI_REFRESH_URL,
  LOCALAPI_TOKEN_URL,
  LOCALAPI_WALLET_BASE_URL,
} from "./localapi-constants";
...
async function startLogin(): Promise<{ loginUrl: string; state: string }> {
  const res = await apiFetch(LOCALAPI_LOGIN_URL);
  if (!res.ok) throw new Error(`LocalApi login start failed (HTTP ${res.status})`);
  ...
  if (!state || !loginUrl) throw new Error("LocalApi login start returned no state/url");
...
async function pollAuthorized(state: string, signal?: AbortSignal): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const res = await apiFetch(`${LOCALAPI_CHECK_URL}?state=${encodeURIComponent(state)}`);
      if (res.ok) {
  ...
  throw new Error("LocalApi login timed out — please try again");
}

async function exchangeToken(state: string, signal?: AbortSignal): Promise<TokenResponse> {
  const res = await apiFetch(`${LOCALAPI_TOKEN_URL}?state=${encodeURIComponent(state)}`, { signal });
  if (!res.ok) throw new Error(`LocalApi token exchange failed (HTTP ${res.status})`);
  const data = (await res.json()) as TokenResponse;
  if (!asString(data.access_token)) throw new Error("LocalApi token exchange returned no access token");
  return data;
}

async function refreshBrokerToken(refreshToken: string, signal?: AbortSignal): Promise<TokenResponse> {
  const res = await apiFetch(LOCALAPI_REFRESH_URL, {
    method: "POST",
    body: JSON.stringify({ refresh_token: refreshToken }),
    signal,
  });
  if (!res.ok) throw new Error(`LocalApi token refresh failed (HTTP ${res.status})`);
  const data = (await res.json()) as TokenResponse;
  if (!asString(data.access_token)) throw new Error("LocalApi token refresh returned no access token");
  return data;
}
...
const localApiOAuth: OAuthAuth = {
  ...
  async login(interaction: AuthInteraction): Promise<OAuthCredential> {
    ...
    interaction.notify({
      ...
      url: loginUrl,
      instructions:
        "在浏览器中完成 LocalApi 登录并授权。若浏览器没有自动打开，请点击下方链接。",
    });
  ...
  async refresh(credential: OAuthCredential, signal?: AbortSignal): Promise<OAuthCredential> {
    if (!credential.refresh) throw new Error("LocalApi 登录已过期，请重新登录");
...
function channelBaseUrl(channel: Channel | undefined): string {
  return channel === "coding" ? LOCALAPI_CODING_BASE_URL : LOCALAPI_WALLET_BASE_URL;
}
...
/**
 * LocalApi exposes reasoning as { enabled, effort: string[] } per model.
 * Pi-Web models map it to `reasoning` plus a `thinkingLevelMap`: supported
 ...
 * LocalApi also allows an "ultra" effort, which has no pi level — it is
...
async function fetchLocalApiModels(context: RefreshModelsContext): Promise<readonly Model<"openai-completions">[]> {
  const storedModels = (context.stored?.models ?? []).filter(
    (m): m is Model<"openai-completions"> => m.provider === LOCALAPI_PROVIDER_ID && m.api === "openai-completions",
  );
  ...
  try {
    ...
    if (res.status === 402) {
      throw new Error("LocalApi：当前账号没有有效的订阅套餐（Coding Plan）。请先在 LocalApi 购买套餐，或重新登录选择普通 API 通道。");
    }
    if (!res.ok) throw new Error(`LocalApi models failed (HTTP ${res.status})`);
    ...
    for (const entry of data) {
      if (typeof entry === "string" && entry) {
        ...
        const rec = entry as { id: string; name?: unknown; reasoning?: { enabled?: unknown; effort?: unknown } };
        rows.push({
          id: rec.id,
    ...
    if (rows.length === 0) throw new Error("LocalApi returned no models");
    return toModels(rows, baseUrl);
  } catch (error) {
    if (error instanceof Error && error.message.startsWith("LocalApi：")) {
...
export function createLocalApiProvider(): Provider<"openai-completions"> {
  return createProvider({
    id: LOCALAPI_PROVIDER_ID,
    name: LOCALAPI_DISPLAY_NAME,
    baseUrl: LOCALAPI_WALLET_BASE_URL,
    auth: { oauth: localApiOAuth },
    models: [],
    fetchModels: fetchLocalApiModels,
